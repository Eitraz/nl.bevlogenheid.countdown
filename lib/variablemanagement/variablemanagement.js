...
module.exports = {
    ...
    init: function() {
        ...
        variables.forEach(function(variable) {
            let newToken = new Homey.FlowToken(variable.name, {
                type: 'number',
                title: variable.name
            });
            newToken.register()
              .then(() => {
                if (variable.value == 0 || variable.value == -1) {
                   return newToken.setValue(0)
                } else {
                   return newToken.setValue(variable.value)
                }
              })
              .catch( err => {
                this.error( err );
        ...
        Homey.ManagerSettings.on('set', function(action) {
            if (action == 'changedvariables') {
                changeObject = Homey.ManagerSettings.get('changedvariables');
